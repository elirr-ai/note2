...
public class CanvasView extends View {
	...
	private float xloc=100,yloc=100;
	private String text=null;
	int[] colors =new int[]{Color.BLACK,Color.YELLOW,Color.CYAN,Color.GREEN,Color.MAGENTA,Color.RED,
			Color.BLUE,Color.WHITE};
	int colorPointer=0;
	float textSize=35.0f;
	
    private Paint paint,paint1,paint2,paint4,paintEdge,paintPen,paint2text;
//    private Path p;
    ...
    int width,height;
//    boolean f;
    boolean fDrawText=false;
    boolean showPen=true;
    boolean useBitmap=true;
    ArrayList<PointHolder1> pal1 =new ArrayList<PointHolder1>();
    Paint pi;
...
//        paintEdge.setStyle(Paint.Style.STROKE);
        ...
        paint2text = new Paint();
        paint2text.setAntiAlias(true);
        paint2text.setStrokeWidth(2);
        paint2text.setTextSize(textSize);
        paint2text.setStyle(Paint.Style.FILL);
        paint2text.setColor(colors[colorPointer]);
    }
    @Override
    protected void onDraw(Canvas canvas) {
//         canvas.drawRect(0, 0, getMeasuredWidth(), getMeasuredHeight(), paintEdge);
         if (null!=bitmap  )	 canvas.drawBitmap(bitmap, 0, 0, null);
...
//        				(float)(9.95*(width/10)), (float)(19.90*(height/20)),paintPen);
//        		}
        
        if (fDrawText && null!=text && !text.equals("")){
        paint2.setTextSize(textSize);
        paint2.setColor(colors[colorPointer]);
        canvas.drawText(text, xloc, yloc, paint2);
        }
//        f=false;
//        fDrawText=false;
    }  // end of onDraw
...
//    	f=true;
    ...
    public void drawLine1 (ArrayList<PointHolder1> pal){
    	this.pal1=pal;
//   	f=true;
    	invalidate();
    }
...
public void setTextxy(String s, float x,float y, int colorPointer2){
	fDrawText=true;
//	f=true;
	this.colorPointer=colorPointer2;
	this.text=s;
	 this.xloc=x; this.yloc=y;
	 invalidate();
}
